...
#include <TMB.hpp>
// Function for detecting NAs, so we can always put NAs in and get predicted values
template<class Type>
bool isNA(Type x){
...
Type objective_function<Type>::operator() ()
{
  ...
  // SPDE objects
  DATA_SPARSE_MATRIX(M0); // diagonal matrix
  DATA_SPARSE_MATRIX(M1); // first order adjacency of mesh
  DATA_SPARSE_MATRIX(M2); // second order adjacency of mesh
  ...
  // Objective funcction
  using namespace density; // to use built in functions in TMB
  int n_i = c_i.size();
  ...
  // calculate precision matrix for GRMF using components
  Eigen::SparseMatrix<Type> Q = exp(4*ln_kappa)*M0 + Type(2.0)*exp(2*ln_kappa)*M1 + M2;
  
  // for spatial smoother, stick it into GMRF function
  // defines spatial process for omega_s, space effect
  ...
  for( int t=0; t<n_t; t++){
  for( int s=0; s<n_s; s++){
    D_t(t) += a_s(s) * exp(log_d_st(s,t)); // expand density by area for each loc
  }}
