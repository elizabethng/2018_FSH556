...
setwd("~/Documents/GitHub/2018_FSH556/Week 9 -- Movement and network models/Lab 9")
...
# Simulate data
######################
# Use mesh based because TMB uses the mesh--that means there won't be any model misspecification
...
# Grid takes longer to run...because it's misspecified??
# Grid is also finer in space, so that makes it take longer too
...
# NOTE: in MoveList, there are four matrices
names(MoveList)
# recall that we need the matix A that represents the net effect of advection and diffusion
# A = lambda_1*M_north + lambda_2*M_south + lambda_3*M_west + lambda_4*M_east
# if they're all the same, we have diffusive movement
