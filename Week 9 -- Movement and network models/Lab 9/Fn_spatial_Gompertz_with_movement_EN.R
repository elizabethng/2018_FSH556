...
Sim_Fn = function( MoveMat, SD_omega=1, SD_epsilon=1, SD_effort=1, Scale, Dynamical_Model, n_s, n_t, r_s, n_r, loc_r, alpha, beta ){
  ...
  # RMgauss here is a fairly constrained--assumes very smooth field
  # it's going to be hard to get aroudn that assumption for our data
  # TMB always uses Matern becasue that goes with the SPDE with alpha = 2, nu = 1, 
  # exp is matern 1/2, so we're fitting it assuming a single derivate, but simulated
  # from a fucntion with infinite derivatives
  ...
  for(t in 1:n_t){
    ...
    if(t>=2){
      upred_rt[,t] = as.vector( MoveMat %*% u_rt[,t-1] ) # simulation model has deterministic movement
