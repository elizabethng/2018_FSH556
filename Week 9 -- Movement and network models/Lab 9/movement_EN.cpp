...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_INTEGER(n_tdiv);     // number of time steps?
  ...
  PARAMETER_ARRAY( ln_u_gt );            // State-matrix (log density? treated as ranef)
  ...
  // Movement matrix
  // options vec allows us to specify having movement in the model or no
  ...
  // no movment means having an identity movement matrix
  if( Options_vec(0)==0 ){
    M_sparse.setIdentity();
    Identity_sparse.setIdentity();
    Mdiv_sparse.setIdentity();
  }
  // If movment occurs, we use the euler approximation
...
//   Random field probability
  ...
  for(int t=1; t<n_t; t++){
    for(int r=0; r<n_r; r++){ // get the predicted densities
      u_rt(r,t) = exp( ln_u_gt(r,t) );
      upred_rt(r,t) = u_rt(r,t-1);
    }
    for(int tdev=0; tdev<n_tdiv; tdev++){  // Euler approximation implemented?
      upred_rt.col(t) = Mdiv_sparse * upred_rt.col(t).matrix();
    }
    for(int r=0; r<n_r; r++){ // switch between Gompertz and Ricker
  ...
  // Probability
  for(int t=0; t<n_t; t++){ // epsilon is the spatial process error in the state-space part
  ...
  REPORT( NLL_i );
  REPORT( Omega_g );
  REPORT( Range_raw );  // decoreelation distance
  REPORT( SigmaU );
  REPORT( SigmaO );
  REPORT( tauU );
  REPORT( tauO );
  REPORT( NLL_c );
  REPORT( NLL );
  REPORT( rho );      // density dependence in gompertz model
  REPORT( log_mvec ); // these are the "lambdas" in the movment model
  REPORT( mvec );
