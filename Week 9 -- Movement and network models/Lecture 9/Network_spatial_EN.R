
library(TMB)
setwd( "C:/Users/James.Thorson/Desktop/Project_git/2018_FSH556/Week 9 -- Movement and network models/Lecture 9" )

# Network parentage
set.seed(1)
parent_b = c( NA, 1, 2, 3, 3, 5, 5, 5, 8, 9, 10 )
dist_b = c( NA, rlnorm(length(parent_b)-1, meanlog=0, sdlog=1) )
DF_b = cbind( parent_b=parent_b, child_b=1:length(parent_b), dist_b=dist_b )
b_i = 1:nrow(DF_b)

# Distance matrix
D = array(NA, dim=rep(length(dist_b),2) )
D[ na.omit(DF_b)[,c("parent_b","child_b")] ] = na.omit(DF_b)[,'dist_b']
D[ na.omit(DF_b)[,c("child_b","parent_b")] ] = na.omit(DF_b)[,'dist_b']
Dsparse = as(ifelse(is.na(D),0,D),"dsCMatrix")

sparse_exp = function(A){
  B = exp(as.matrix(A))
  B = ifelse( B==1, 0, B )
  return( as(B,"dsCMatrix") )
}
exp_neg_D = sparse_exp(-D)

# Parameter values
Rho=0.3           # Correlation at distance = 1
SDmarg=0.3
theta = -log(Rho)  # Decorrelation rate per distance
log_mean = 4
detect_prob = 0.99
Npass = 1

# Detection probability for each pass
detect_per_pass = rep(NA,Npass)
for( zI in 1:Npass ){
  detect_per_pass[zI] = detect_prob * (1-detect_prob)^(zI-1)
}

# Derived
SDinput = SDmarg * sqrt(2*theta)
SDcond = SDmarg * sqrt(1-Rho^2)

# Simulate random effect
# Sweeps through probability of the stream network
omega_b = rep(NA, length(b_i))
pow = function(a,b) a^b
Type = function(a) a
rho_b = SDinput_b = rep(NA, length(b_i))
for( b in 1:length(omega_b) ){# spatial variable to simulate
  if( is.na(dist_b[b]) ){
    # Correlation between i and parent(i) as distance -> INF
    rho_b[b] = 0;
    # SD of Ornstein-Uhlenbeck process as distance -> INF
    SDinput_b[b] = SDinput / pow(2*theta, 0.5);
    # conditional probability
    epsilon_b[b] = rnorm(n=1, Type(0.0), SDinput_b[b] );
  }
  if( !is.na(dist_b[b]) ){
    # Correlation between i and parent(i)
    rho_b[b] = exp(-theta * dist_b[b]);
    # SD of O-U process
    SDinput_b[b] = pow( pow(SDinput,2)/(2*theta) * (1-exp(-2*theta*dist_b[b])), 0.5 );
    # conditional probability
    omega_b[b] = rnorm(n=1, rho_b[b]*omega_b[parent_b[b]], SDinput_b[b] );
  }
}

# Simulate data
lambda_i = exp( omega_b + log_mean ) # high mean --> low cv for poisson (one way to increase "info" in data for small example)
c_iz = matrix( rpois(n=length(lambda_i)*Npass, lambda=outer(lambda_i,detect_per_pass[1:Npass])), ncol=Npass )
#c_iz[] = NA # count i and pass z

# Format inputs for TMB
Data = list( "Options_vec"=c(0), "c_iz"=c_iz, "b_i"=b_i-1, "parent_b"=DF_b[,'parent_b']-1, "child_b"=DF_b[,'child_b']-1, "dist_b"=DF_b[,'dist_b'] )
Params = list( "log_theta"=log(theta), "log_SD"=log(SDinput), "log_mean"=log(1), "logit_detect_prob"=0, "Epsiloninput_b"=rep(0,max(b_i)) )
Random = "Epsiloninput_b"

# Turn off detect_prob if single-pass model
Map = list()
if( Npass==1 ){
  Map[["logit_detect_prob"]] = factor(NA)
  Params[["logit_detect_prob"]] = qlogis(0.9999)
}

# Compile TMB
compile( "network_spatial.cpp" )
dyn.load( dynlib("network_spatial") )

##################
# Check that precision matrix is correct
##################
# If there's no data in the model, jnll is equal to the prior data, so hessian of the inner optimizer is Q

# Extract from TMB
Data[["c_iz"]][] = NA # only thing in JNLL is prob of spatial process
Obj = MakeADFun( data=Data, parameters=Params, random=Random )
Hess = Obj$env$spHess( random=TRUE ) # hessian of inner optimizer is equal to Q of the spatial process

# Calculate analytically
# THat Jim worked out from plug-and-play based on the structure he knew it had to have
Dfilled = ifelse(is.na(D), Inf, D)
# basically the formula for a CAR model
# not written anywhere because CAR models are usually area model (no distances) or point
# count models (but with equal spacing), so not a lot of examples of the unequally spaced point
# count model
Q = -exp(-theta*Dfilled) / (SDmarg*sqrt(1-exp(-theta*Dfilled)^2))^2
diag(Q) = 1/SDmarg^2 + colSums(exp(-2*theta*Dfilled) / (SDmarg*sqrt(1-exp(-theta*Dfilled)^2))^2)

# Compare
summary( as.vector(Q) - as.vector(Hess) )


##############
# Compare estimates from two methods
##############

######### Version 0:  Sweep upstream to downstream
Data = list( "Options_vec"=c(0), "c_iz"=c_iz, "b_i"=b_i-1, "parent_b"=DF_b[,'parent_b']-1, "child_b"=DF_b[,'child_b']-1, "dist_b"=DF_b[,'dist_b'] )
Obj = MakeADFun( data=Data, parameters=Params, random=Random, map=Map )
Opt0 = TMBhelper::Optimize( obj=Obj, newtonsteps=1 )
Report0 = Obj$report()

######### Version 1:  Joint probability
Data = list( "Options_vec"=c(1), "c_iz"=c_iz, "b_i"=b_i-1, "parent_b"=DF_b[,'parent_b']-1, "child_b"=DF_b[,'child_b']-1, "dist_b"=DF_b[,'dist_b'] )
Obj = MakeADFun( data=Data, parameters=Params, random=Random, map=Map )
Opt1 = TMBhelper::Optimize( obj=Obj, newtonsteps=1 )
Report1 = Obj$report()

# Compare MLE from two methods
rbind( "True"=c(log(theta),SDinput,log_mean), "Method1"=Opt0$par, "Method2"=Opt1$par )
# spatial parameter estimates not that great/imprecise
# NOTE defining the precision matrix ends up being a bit faster! (but less than twice as fast)

# Can also set some confluence nodes (3 and 5) to zero, since they may just be added there for convenience
# and TMB will predict values for those locations
# And note that if we do that, the SEs for those points will have higher variance
# than other points WITH data, but not as high as the marginal variance because of shrinkage


# Bias Correction
# retransformation bias--logit linked linear predictor
# random effects have nonzero standard error. so nonlinear transformation thereof will not be same 
# B = exp(A) --> mena of A = mu, median of B = exp(mu), but need bias correction for mean of B
# previous claim before this: krig in normal space (linear) only
# ADREPORT( lambda_i ) to get bias correction to work

# unbiased predictions for lambda_i--> includes exp(spatial variable) so we need to correct for retransformation bias
# need to adreport that to get the calculated SEs from TMB
# look at summary( Opt$SD, "report ) which will have a new column, estimate bias correct
# here, always looks a bit high, which is a property of exponentiation process
# (just like lognormal dist, mean is higher than median)
# bias correction is increasing function of SE and the second derivative of the distribitons (of ?)

# Q: different from REML issue, which we had used for the SE of the fixed effects?





