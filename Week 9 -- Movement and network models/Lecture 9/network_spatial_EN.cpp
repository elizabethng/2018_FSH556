...
Type objective_function<Type>::operator() ()
{
  ...
  // almost identical to way we simulated the data
  if( Options_vec(0)==0 ){
  ...
  if( Options_vec(0)==1 ){
    ...
    for(int b=1; b<n_b; b++){
      // filling in entries in sparse matrix using commands in Eigen
      // stored internall as 3 column -- 1 col of rownumber, 1 col of col numbers, 1 col of values
      // cant do operations on sparse matrices like in R, so need to be more verbose
    ...
    REPORT( Q );
    //put calculated Q matrix in the density function
  ...
  for (int z=0; z<n_z; z++){
    ...
    if( !isNA(c_iz(i,z)) ){
      jnll -= dpois(c_iz(i,z), lambda_i(i)*detect_per_pass(z), true); // hidden trick--multiply density by detection probability (is this like an offset??)
    }
  }}
