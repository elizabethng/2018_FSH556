...
Type objective_function<Type>::operator() ()
{
  ...
  if( Options_vec(0)==0 ){
    // could put more complicated eq for variance of first site (it has not upstream location)
    // like conditioning on a point observed infinitely far upstream
  ...
  // populate the whole covariance matrix
  // this is slower, potentially because of MVNORM??
  if( Options_vec(0)==2 ){
    ...
    REPORT( Cov_ss );
    // MVNROM is mean 0 multinormal dist
    // recall that they return the NEGATIVE ll by default
    // lives in namespace density package
    //                  covariance    mean
