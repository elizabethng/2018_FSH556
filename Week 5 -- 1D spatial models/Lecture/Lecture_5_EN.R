...
Sigma2 = 2 ^ 2
x = 1:10
Dist = outer(x, x, FUN=function(a,b){abs(a-b)}) # same as dist()

# Calculate Q directly
Cov = Sigma2 / (1-Rho^2) * Rho^Dist
Q = solve(Cov) # precision matrix
...
# Q and Q2 vary by just only one cell ("boundary effect")
# At boundary solve(Q2) is slightly less variable

###################
# Equal distance autoregressive
###################

x = 1:100 # 100 locations
Rho = 0.8
Sigma2 = (0.5) ^ 2
n_rep = 3 # 3 obs per site
beta0 = 3
...
# recursive formulation/calcualtion for epsilon above
...
# use logit rho to constrain from 0 to 1, to keep things stationary
# bounds vs transformation?
# optimizers assume same scale for params, so want things on the same scale
# here, logit more likely to be stable

# options vec is different tmb formulations
...
Opt0$run_time # 0.13
Opt1$run_time # 0.11
Opt2$run_time # 1.08 MVN has 10-fold increase in time
Opt3$run_time # 0.11

# Compare results
cbind(par0, par1, par2, par3)
# pretty similar, but par1 is slightly different b/c of boundary effects (b/c of shortcut CAR model hand-coding)
...
# by looking for blocks in hessian
# all look very similar, negatives on off diag and positives on diag
# 3/4 have big expansive white (seperable, 0 things)
# version 2 has lots of grey boxes, no seperability detected
