...
//multivariate normal functions similar to R's version
//but it takes Q (precision matrix) instead of Sigma
template<class Type>
...
Type objective_function<Type>::operator() ()
{
  ...
  vector<Type> jnll_comp(2);  //jnll now has two components to keep ranef and fixef separated for debugging
  jnll_comp.setZero();
  ...
  //use MVNORM in namespace density() which is like loading a library
  // this one runs slower because tracking too many elements of Hessian
  // could be that MVNORM is not designed to do that (assumes cov is dense)
  // bigger itneresting Q, how did it detect sparseness of others?
  if( Options_vec(0)==2 ){
    ...
    REPORT( Cov_ss );
    //function where you give covariance, and it returns a fucntion that you can call
    // with input of epsilon
    // unlike in R, these density functions return the NEGATIVE LOG LIKELIHOOD (so add)
    jnll_comp(1) += MVNORM(Cov_ss)( epsilon_s );
  }
  //// Calculate using built-in TMB functions
  // AR1(rho) retruns standard normal for first order autoregressive process,
  // wont have covariance that we want, so we need to use the SCALE function
  // with the marginal standard deviation
  // AR1 and SCALE also returns negative log like
  if( Options_vec(0)==3 ){
