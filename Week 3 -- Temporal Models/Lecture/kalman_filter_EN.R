...
setwd("~/Documents/GitHub/2018_FSH556/Week 3 -- Temporal Models/Lecture")
Use_REML = TRUE # FALSE # turn on REML, tells function to treat x_t as random and other variances too. we'll still make inference about fixed effects, but we'll estimate them as random
set.seed(2)
...
# Get the Hessian using spHess (the sparse Hessian)
Hess = Obj$env$spHess( random = TRUE)
Hess[1:6, 1:6]
# The dots mean "structural zeroes", where partial covariance is zero
# Because of conditional independence! due to markov property?
# This class is possible becasue TMB automatically detects the sparsity
# Hess is a tridiagonal Matrix
library(INLA)
image(Hess)
# Tridiagonal but we're only using 2D integrals? What about that?
# one way to think about it...matrix is symmetric, so we're only using sort of half of it
