...
# attach("EBSBTS_download.Rdata")
# test = Downloaded_data
# rm(Downloaded_data)

# If we had a simple random sampling design, we could get an index by just looking at av
B_t = tapply( CPUE[,'Wt'], INDEX=CPUE[,'Year'], FUN=mean )

  
# Get the annual standard errors
samp_size = plyr::count(CPUE, "Year")$freq
sd_bt = aggregate(Wt ~ Year, CPUE, sd)$Wt
SE_bt = sd_bt/sqrt(samp_size)

CV = SE_bt/B_t
sig2_log_bt = log(CV^2 + 1)



compile("gompertz_EN.cpp")
dyn.load( dynlib("gompertz_EN") )


# Run Gompertz model again
Data = list( "nt"=length(B_t), "log_b_t"=log(B_t) , "sig2_log_bt" = sig2_log_bt)
Parameters = list( "log_d0"=0, "log_sigmaP"=1, "log_sigmaM"=1, "alpha"=0, "rho"=0, "log_d_t"=rep(0,Data$nt) )
Obj = MakeADFun(data=Data, parameters=Parameters, random=Random, DLL="gompertz")  #
