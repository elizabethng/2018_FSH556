...
# Settings
First chose an example data set for this script, as archived with package (list of datasets distributed with the package)
...
## Spatial settings
The following settings define the spatial resolution for the model, and whether to use a grid or mesh approximation. [mesh = triangluated spde method mesh]
```{r}
...
## Model settings
The following settings define whether to include spatial and spatio-temporal variation, whether its autocorrelated, and whether there's overdispersion [ss models, fieldconfig is which are spatial, spatio temporal etc. this is seeting 1==TRUE so leave them all in. rho config is for correlation, overdispersion both off. obsmodel is for lognormal or gamma uses traditional delta or poisson link]
```{r, tidy=TRUE}
...
## Potential outputs
The following settings define what types of output we want to calculate [derived quantities, turn on and off]
```{r, tidy=TRUE}
...
We also define any potential stratification of results, and settings specific to any case-study data set [stratify the results, calculate density field, eg index of abundance for OR, WA, CA (post stratify results)]
...
We also generate the extrapolation grid appropriate for a given region.  For new regions, we use `Region="Other"`. [think about the footprint of the sampling design for extrapolation. for "other" put in sampled locations and define it with a buffer. there is another function so user can put in a data frame that represented a grid. see vast issue tracker for more info]
...
And we finally generate the information used for conducting spatio-temporal parameter estimation, bundled in list `Spatial_List` [takes extrapolation list and other stuff and runs kmeans and uses that to decide 1000 locations of the knots to model]
...
To estimate parameters, we first build a list of data-inputs used for parameter estimation.  `Data_Fn` has some simple checks for buggy inputs, but also please read the help file `?Data_Fn`. [build all the objects in the nice tMB list format. ]  
...
Next, we use a gradient-based nonlinear minimizer to identify maximum likelihood estimates for fixed-effects [bias correction is memory intensive for some models becaseu it can break the sparsity. so you can select a subset of parameter/derived quantities to bias correct. here, just doing the index]
